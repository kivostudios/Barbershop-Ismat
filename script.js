...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Lightbox (galeri.html uses its own inline script, this handles legacy) ----
  ...
  function initWizard() {
    ...
    function renderCalendar() {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (isPast) { btn.classList.add('disabled'); btn.disabled = true; }
    ...
    async function getBookedTimes(dateStr) {
      ...
      const { data } = await db.from('bookings').select('time').eq('date', dateStr).neq('status', 'cancelled');
      return new Set((data || []).map(b => b.time));
    }
    ...
    async function renderTimes() {
      ...
      morning.innerHTML = '<span style="opacity:.4;font-size:12px">Laden...</span>';
      afternoon.innerHTML = '';
      evening.innerHTML = '';
      ...
      const endHour = isSunday ? 17 : 19;

      const booked = await getBookedTimes(dateStr);
      morning.innerHTML = '';
      ...
      for (let h = startHour; h < endHour; h++) {
        for (let m = 0; m < 60; m += 30) {
          ...
          if (booked.has(time)) { btn.disabled = true; btn.classList.add('booked'); }
          ...
          if (h < 13) morning.appendChild(btn);
          else if (h < 16) afternoon.appendChild(btn);
    ...
    btnConfirm.addEventListener('click', async () => {
      ...
      if (db) {
        ...
        if (error) {
          ...
          btnConfirm.textContent = 'Afspraak Bevestigen';
          return;
        }
      }
      ...
      indicators.forEach(ind => { ind.classList.remove('active'); ind.classList.add('completed'); });
